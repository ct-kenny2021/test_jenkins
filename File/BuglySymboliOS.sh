...
if [ $CONFIGURATION = "Release" ]; then
    # #
    BUGLY_APP_ID='35064d10dc'
    BUGLY_APP_KEY='a0a114c4-13db-4a66-9eb9-43edf5718fb7'
    BUNDLE_IDENTIFIER='com.hadlinks.352AirCleaner'
    UPLOAD_DSYM_ONLY=0
    # #
else
    # #
    BUGLY_APP_ID='be74e3c38b'
    BUGLY_APP_KEY='23785aac-7a22-4460-be6c-d95e94dd025c'
    UPLOAD_DSYM_ONLY=0
    # #
    if [ $CONFIGURATION = "Test" ]; then
        BUNDLE_IDENTIFIER='com.bugu.352AirTest'
    elif [ $CONFIGURATION = "Beta" ]; then
        BUNDLE_IDENTIFIER='com.bugu.352AirBeta'
    else
        BUNDLE_IDENTIFIER='com.bugu.352AirDebug'
    fi
    # #
fi
...
BUGLY_SYMBOL_JAR_PATH="File/BuglySymboliOS.jar"
# 查找添加到系统目录的jar工具
...
function runInXcode(){
    ...
    BUNDLE_VERSION=$(/usr/libexec/PlistBuddy -c 'Print CFBundleVersion' "${TARGET_BUILD_DIR}/${INFOPLIST_PATH}")
    BUNDLE_SHORT_VERSION=$(/usr/libexec/PlistBuddy -c 'Print CFBundleShortVersionString' "${TARGET_BUILD_DIR}/${INFOPLIST_PATH}")
